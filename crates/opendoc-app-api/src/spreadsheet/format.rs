...
use crate::{civil_from_days, days_from_civil, trim_number};
...
fn parsed(kind: &'static str, value: impl Into<String>, number_format: Option<&str>) -> ParsedInput {
    ParsedInput {
        kind,
        value: value.into(),
...
fn parse_time_text(text: &str) -> Option<(f64, &'static str)> {
    ...
    if parts.iter().any(|part| part.is_empty() || !part.chars().all(|ch| ch.is_ascii_digit() || ch == '.')) {
        return None;
    }
    let mut hour = parts[0].parse::<f64>().ok()?;
    let minute = parts[1].parse::<f64>().ok()?;
    let second = parts.get(2).map(|part| part.parse::<f64>().ok()).unwrap_or(Some(0.0))?;
...
pub fn date_serial(year: i32, month: u32, day: u32) -> Option<f64> {
    ...
    let (check_year, check_month, check_day) = civil_from_days(days + days_from_civil(1899, 12, 30));
...
pub fn format_general(value: f64) -> String {
    ...
    if magnitude >= 1e21 || magnitude < 1e-7 {
        ...
        return format!("{mantissa}E{}{:02}", if exponent < 0 { "-" } else { "+" }, exponent.abs());
    }
...
fn parse_number_section(section: &str) -> NumberSection {
    ...
    while let Some(ch) = chars.next() {
        match ch {
            ...
            '.' if !in_fraction && (seen_core || chars.peek().is_some_and(|next| matches!(next, '0' | '#' | '?'))) => {
                seen_core = true;
                in_fraction = true;
            }
            '%' => {
...
fn format_number_section(value: f64, section: &NumberSection, locale: &Locale) -> String {
    ...
    if let Some(exponent_digits) = section.scientific {
        ...
        body.push_str(&format!("{:0width$}", exponent.abs(), width = exponent_digits));
    ...
    if int_part.len() < section.int_digits {
        int_part = format!("{}{int_part}", "0".repeat(section.int_digits - int_part.len()));
    }
    if int_part.is_empty() && section.int_digits == 0 && frac_part.chars().all(|ch| ch == '0') && section.frac_max == 0 {
...
fn tokenize_date_pattern(pattern: &str) -> Vec<DateToken> {
    ...
    while index < chars.len() {
        ...
        let upper_rest: String = chars[index..].iter().take(5).collect::<String>().to_ascii_uppercase();
...
pub fn format_date_pattern(serial: f64, pattern: &str) -> String {
    ...
    let has_meridiem = tokens.iter().any(|token| matches!(token, DateToken::Meridiem(_)));
