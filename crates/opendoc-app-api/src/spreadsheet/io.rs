...
use crate::{
    column_axis, column_to_number, normalize_cell_range, number_to_column, row_axis,
    split_cell_address, AppApiError, AppCell, AppNamedRange, AppSheet, AppSheetMerge,
...
pub fn blank_sheet(id: &str, title: &str, rows: u32, columns: u32) -> AppSheet {
...
pub fn parse_delimiter(value: Option<&str>) -> Result<u8, AppApiError> {
    ...
    let delimiter = match value {
        ...
        other => {
            ...
            let first = chars
                .next()
                .ok_or_else(|| AppApiError::Format("csv delimiter is empty".to_string()))?;
...
pub fn parse_csv(text: &str, delimiter: u8) -> Result<Vec<Vec<String>>, AppApiError> {
    ...
    for record in reader.records() {
        let record = record.map_err(|err| AppApiError::Import(format!("invalid csv: {err}")))?;
...
pub fn export_csv(sheet: &AppSheet, delimiter: u8, locale: &Locale) -> Result<String, AppApiError> {
    ...
    for row in 1..=max_row {
        ...
        writer
            .write_record(&record)
            .map_err(|err| AppApiError::Format(format!("csv export failed: {err}")))?;
    }
    let bytes = writer
        .into_inner()
        .map_err(|err| AppApiError::Format(format!("csv export failed: {err}")))?;
    String::from_utf8(bytes).map_err(|err| AppApiError::Format(format!("csv export failed: {err}")))
}

pub fn decode_base64(text: &str) -> Result<Vec<u8>, AppApiError> {
    let cleaned: String = text.chars().filter(|ch| !ch.is_whitespace()).collect();
    base64::engine::general_purpose::STANDARD
        .decode(cleaned.as_bytes())
        .map_err(|err| AppApiError::Format(format!("invalid base64: {err}")))
}
...
pub fn import_xlsx(bytes: &[u8], title: &str) -> Result<AppSpreadsheetWorkbook, AppApiError> {
    let cursor = Cursor::new(bytes.to_vec());
    let mut reader: calamine::Xlsx<_> = calamine::open_workbook_from_rs(cursor)
        .map_err(|err| AppApiError::Import(format!("invalid xlsx: {err}")))?;
    let _ = reader.load_merged_regions();
    let sheet_names = reader.sheet_names();
    if sheet_names.is_empty() {
        return Err(AppApiError::Import("xlsx workbook has no sheets".to_string()));
    }
    ...
    for (index, name) in sheet_names.iter().enumerate() {
        let range = reader
            ...
            .map_err(|err| AppApiError::Import(format!("invalid xlsx sheet {name}: {err}")))?;
        ...
        for (row, col, value) in range.used_cells() {
            ...
            let mut cell = match value {
                Data::Empty => continue,
                Data::Int(value) => AppCell::new(&address, "number", &value.to_string()),
                Data::Float(value) => AppCell::new(&address, "number", &crate::trim_number(*value)),
                Data::String(text) => AppCell::new(&address, "string", text),
                Data::Bool(flag) => AppCell::new(&address, "bool", if *flag { "true" } else { "false" }),
                Data::DateTime(value) => {
                    let serial = value.as_f64();
                    let mut cell = AppCell::new(&address, "number", &crate::trim_number(serial));
                ...
                Data::DateTimeIso(text) | Data::DurationIso(text) => {
                    AppCell::new(&address, "string", text)
                }
                Data::Error(error) => AppCell::new(&address, "string", xlsx_error_code(error)),
            };
        ...
        for (_, _, dimensions) in reader.merged_regions_by_sheet(name) {
            ...
            if let Ok(range) = normalize_cell_range(&range) {
                if range.contains(':') {
                    sheet.merges.push(AppSheetMerge {
...
fn xlsx_format(cell: &AppCell) -> Option<rust_xlsxwriter::Format> {
...
fn xlsx_err(err: rust_xlsxwriter::XlsxError) -> AppApiError {
    AppApiError::Format(format!("xlsx export failed: {err}"))
}
...
pub fn export_xlsx(workbook: &AppSpreadsheetWorkbook) -> Result<Vec<u8>, AppApiError> {
    ...
    for sheet in &workbook.sheets {
        ...
        for label in &sheet.hidden_columns {
            if let Some(col) = column_to_number(label) {
                worksheet.set_column_hidden((col - 1) as u16).map_err(xlsx_err)?;
            }
        }
        ...
        for cell in &sheet.cells {
            ...
            match cell.user_kind.as_str() {
                ...
                "bool" => {
                    ...
                    match &format {
                        Some(format) => worksheet.write_boolean_with_format(row, col, value, format),
        ...
        for merge in &sheet.merges {
            let Ok(range) = crate::parse_cell_range(&merge.range) else {
                continue;
            };
    ...
    for named in &workbook.named_ranges {
        ...
        let absolute = named
            ...
            .collect::<Vec<_>>()
            .join(":");
        let _ = book.define_name(&named.name, &format!("='{}'!{absolute}", title.replace('\'', "''")));
    }
...
pub fn import_defined_names(
    ...
    for (name, formula) in reader.defined_names() {
        ...
        let Ok(name) = crate::normalize_named_range_name(name) else {
            continue;
        };
        out.push(AppNamedRange {
