...
use crate::{
    column_to_number, graph_dependency_target, graph_dependent_label, number_to_column,
    split_cell_address, AppCell, AppCellDependency, AppNamedRange, AppSheet,
    AppSpreadsheetWorkbook, AppWarning,
};
...
pub struct AppSpreadsheetEvaluationContext {
    pub now_ms: u64,
    pub seed: u64,
}
...
fn structure_fingerprint(workbook: &AppSpreadsheetWorkbook) -> String {
...
fn build_snapshot(workbook: &AppSpreadsheetWorkbook) -> RecalcSnapshot {
...
impl<'a> WorkbookEnv<'a> {
    fn new(
        ...
        for range in named_ranges {
            ...
            let (start, end) = range.range.split_once(':').unwrap_or((&range.range, &range.range));
    ...
    fn cell(&self, key: CellKey) -> Option<&'a AppCell> {
    ...
    fn source_value(cell: &AppCell) -> FormulaValue {
...
fn detect_mode(workbook: &AppSpreadsheetWorkbook, env: &WorkbookEnv<'_>) -> RecalcMode {
...
pub fn evaluate_workbook(workbook: &mut AppSpreadsheetWorkbook) -> Vec<AppWarning> {
            ...
            for sheet in &sheets_snapshot {
                ...
                for cell in &sheet.cells {
                    ...
                    if !outcome.spilled.contains_key(&key)
                        && outcome.previous_values.get(&key).is_some_and(|v| !v.is_blank())
                    {
                        changed = true;
                    }
                }
    ...
    let sheet_ids: Vec<String> = workbook.sheets.iter().map(|sheet| sheet.id.clone()).collect();
            ...
            sheet_ids
                ...
                .is_some_and(|sheet_index| {
                    !evaluated_addresses.contains(&(sheet_index, address.to_string()))
                        && workbook.sheets[sheet_index].cells.iter().any(|cell| {
                            cell.address == address && cell.computed_value == "#REF!"
                        })
                })
        });
    ...
    for (sheet_index, sheet) in workbook.sheets.iter_mut().enumerate() {
        ...
        let new_targets: HashMap<String, (String, FormulaValue)> = spilled_by_sheet
            ...
            .map(|targets| {
                targets
                    .iter()
                    .map(|(key, anchor, value)| {
                        (key.address(), (anchor.address(), value.clone()))
                    })
                    .collect()
            })
            .unwrap_or_default();
...
/// cells.
pub fn refresh_display_values(workbook: &mut AppSpreadsheetWorkbook) {
    ...
    addresses
        ...
        .map(|(sheet_id, address)| {
            ...
            let direct_dependents = dependents
                ...
                .into_iter()
                .collect::<Vec<_>>();
            let invalidation_order = collect_invalidation_order(sheets, &sheet_id, &address, &dependents);
            AppCellDependency {
                sheet_id,
                address,
