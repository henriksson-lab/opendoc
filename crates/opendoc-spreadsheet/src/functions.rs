...
use crate::functions_legacy::{
    clean_text, datedif_value, encode_formula_base_digits, evaluate_function,
    formula_source_is_date_producing, is_supported_email_address, is_supported_url,
    networkdays_value_with_holidays, parse_formula_datevalue, parse_formula_decimal,
    parse_formula_numbervalue, parse_formula_text_number, parse_formula_timevalue,
    proper_case_text, wildcard_search_position, workday_value_with_holidays,
};
use crate::functions_legacy_tail::{
    correlation_value, covariance_value, forecast_value, regression_value,
};
use crate::wildcard_text_matches;

pub(super) type FResult = Result<FormulaValue, FormulaError>;

/// Serial number of the Unix epoch in the 1899-12-30 date system.
pub const UNIX_EPOCH_SERIAL: f64 = 25569.0;
const MS_PER_DAY: f64 = 86_400_000.0;

pub(super) fn legacy(result: Result<f64, String>) -> FResult {
    match result {
...
pub(super) fn number(value: f64) -> FResult {
    if value.is_finite() {
...
pub(super) fn arity(args: &[Expr], min: usize, max: usize) -> Result<(), FormulaError> {
...
fn lift_over_arrays(
    ...
    Ok(Some(FormulaValue::Array(FormulaArray::new(
        rows, cols, out,
    ))))
}
...
fn call_values(ev: &mut Evaluator<'_>, name: &str, values: Vec<FormulaValue>) -> FResult {
    match name {
        ...
        "DOLLAR" => {
            ...
            let locale = Locale::for_tag(&ev.env.locale());
            let formatted =
                format::format_fixed(value.abs(), decimals.max(0) as usize, true, &locale);
        ...
        "COUNTBLANK" => {
            ...
            for value in &values {
                for item in flatten(value) {
                    if item.is_blank()
                        || matches!(item, FormulaValue::Text(text) if text.is_empty())
                    {
                        count += 1.0;
                    }
                }
            ...
            for index in 0..length {
                ...
                for array in &arrays {
                    product *= match &array[index] {
                        FormulaValue::Number(value) => *value,
                        FormulaValue::Error(error) => return Err(*error),
                        FormulaValue::Bool(true) => 1.0,
                        FormulaValue::Bool(false) => 0.0,
                        _ => 0.0,
                    };
                }
                total += product;
...
fn cell_state_predicate(ev: &mut Evaluator<'_>, name: &str, args: &[Expr]) -> FResult {
    ...
    match ev.resolve_range_expr(&args[0]) {
        Ok(range) => {
            let meta = ev
                .env
                .cell_meta(range.sheet, range.col_start, range.row_start);
            Ok(FormulaValue::Bool(match name {
                "ISBLANK" => {
                    meta.is_none_or(|cell| cell.user_kind == "empty" && cell.spill_source.is_none())
                }
...
fn address_function(ev: &mut Evaluator<'_>, args: &[Expr]) -> FResult {
    ...
    Ok(FormulaValue::Text(match sheet {
        Some(sheet) if !sheet.is_empty() => {
            format!("{}!{body}", formula_sheet_title_prefix(&sheet))
        }
        _ => body,
    }))
}
...
fn split_function(values: &[FormulaValue]) -> FResult {
    ...
    if delimiter.is_empty() {
        return Ok(FormulaValue::Array(FormulaArray::row(vec![
            FormulaValue::Text(text),
        ])));
    }
...
fn conditional_aggregate(ev: &mut Evaluator<'_>, name: &str, args: &[Expr]) -> FResult {
    ...
    let sum_sheet = sum_range
        .map(|range| range.sheet)
        .unwrap_or(criteria_range.sheet);
    let mut total = 0.0;
    let mut count = 0.0;
...
fn multi_conditional_aggregate(ev: &mut Evaluator<'_>, name: &str, args: &[Expr]) -> FResult {
    ...
    let aggregate_cells: Option<Vec<(u32, u32)>> =
        aggregate_range.map(|range| range.cells().collect());
